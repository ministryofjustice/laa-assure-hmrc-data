...
RSpec.describe HmrcInterfaceResultWorker, type: :worker do
  ...
  describe ".sidekiq_retries_exhausted" do
    ...
    it "updates status of submission to \"exhausted\"" do
      ...
      }.to change { submission.reload.status }.from("submitted").to("exhausted")
    end

    it "send failure message to sentry" do
      config.sidekiq_retries_exhausted_block.call(job, exc)
      expect(Sentry).to have_received(:capture_message).with(
        /Failed #{job["class"]} for submission \["#{submission.id}"\]: oops, I did it again!.*/
      )
    end
  end
  ...
  describe ".perform_async" do
    context "when queue set by caller" do
      subject(:perform_async) do
        described_class.set(queue:).perform_async(submission.id)
      end
  ...
  describe "#perform" do
    ...
    before { allow(HmrcInterfaceResultService).to receive(:call) }

    it_behaves_like "application worker logger"

    it "calls HmrcInterfaceResultService with submission id" do
      perform
      expect(HmrcInterfaceResultService).to have_received(:call).with(
        submission.id
      ).once
    end
  end
