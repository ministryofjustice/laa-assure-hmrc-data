...
RSpec.describe "sidekiq-scheduler config" do
  ...
  let(:schedules) { YAML.load_file(sidekiq_file).dig(:scheduler, :schedule) || [] }
  ...
  context "when loading schedules" do
    it "all have valid cron syntax", :aggregate_failures do
      schedules.each do |name, config|
        ...
        expect { Fugit.do_parse(cron) }.not_to raise_error, "cron syntax for #{name}, \"#{cron}\", is invalid"
      end
    end
    ...
    it "all have valid job/worker class", :aggregate_failures do
      schedules.each do |name, config|
        ...
        expect { klass.constantize }.not_to raise_error, "class for #{name}, \"#{klass}\", does not exist"
      end
    end
  end
end
