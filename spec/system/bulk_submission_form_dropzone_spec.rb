  ...
  context "with an authorised user" do
    ...
    context "with an acceptable file" do
      ...
      it "user can upload and delete a CSV from a bulk submission" do
        ...
        expect(page).to have_selector(
          ".govuk-hint",
          text: "The maximum file size is 1MB. Files must be a CSV."
        )
      ...
      it "user can upload and update a CSV on a bulk submission" do
        ...
        expect(page).to have_current_path(
          edit_bulk_submission_form_path(BulkSubmission.last.id)
        )
      ...
      it "renders summary and field level errors" do
        ...
        expect(page).to have_selector(
          ...
          text: "There is a problem"
                ...
                ".govuk-error-summary__body",
                text: "empty.csv is empty"
              ).and have_selector(
                      ".govuk-error-message",
                      text: "empty.csv is empty"
                    )

        find(".dz-clickable").drop(
          file_fixture("invalid_content_type_png_as_csv.csv")
        )
        expect(page).to have_selector(
          ".govuk-error-summary__body",
          text: "invalid_content_type_png_as_csv.csv must be a CSV"
        ).and have_selector(
                ".govuk-error-message",
                text: "invalid_content_type_png_as_csv.csv must be a CSV"
              )
        ...
        expect(page).to have_selector(
          ".govuk-error-summary__body",
          text: "one_byte_too_big.csv is more than 1MB"
        ).and have_selector(
                ".govuk-error-message",
                text: "one_byte_too_big.csv is more than 1MB"
              )
        ...
        expect(page).to have_selector(
          ".govuk-error-summary__body",
          text: "empty.png must be a CSV"
        ).and have_selector(
                ".govuk-error-summary__body",
                text: "empty.png is empty"
              ).and have_selector(
                      ".govuk-error-message",
                      text: "empty.png must be a CSV"
                    ).and have_selector(
        ...
        expect(page).to have_selector(
          ".govuk-error-summary__body",
          text: "too_many_rows.csv has more than 35 records"
        ).and have_selector(
                ".govuk-error-message",
                text: "too_many_rows.csv has more than 35 records"
              )
        ...
        expect(page).to have_selector(
          ".govuk-error-summary__body",
          text: "unparseable_file.csv unable to read file"
        ).and have_selector(
                ".govuk-error-message",
                text: "unparseable_file.csv unable to read file"
              )
        ...
        expect(page).to have_selector(
          ".govuk-error-summary__body",
          text: "invalid_headers.csv has invalid headers"
        ).and have_selector(
                ".govuk-error-message",
                text: "invalid_headers.csv has invalid headers"
              )
        ...
        expect(page).to have_selector(
          ".govuk-error-summary__body",
          text: "invalid_content.csv first name missing at row 2"
        ).and have_selector(
                ".govuk-error-summary__body",
                text: "invalid_content.csv last name missing at row 2"
              ).and have_selector(
                      ".govuk-error-summary__body",
                      text: "invalid_content.csv invalid date of birth at row 2"
                    ).and have_selector(
                            ".govuk-error-summary__body",
                            text:
                              "invalid_content.csv invalid national insurance number at row 2"
                          ).and have_selector(
                                  ".govuk-error-summary__body",
                                  text:
                                    "invalid_content.csv invalid period start date at row 2"
                                ).and have_selector(
                                        ".govuk-error-summary__body",
                                        text:
                                          "invalid_content.csv invalid period end date at row 2"
                                      ).and have_selector(
                                              ".govuk-error-message",
                                              text:
                                                "invalid_content.csv first name missing at row 2"
                                            ).and have_selector(
                                                    ".govuk-error-message",
                                                    text:
                                                      "invalid_content.csv last name missing at row 2"
                                                  ).and have_selector(
                                                          ".govuk-error-message",
                                                          text:
                                                            "invalid_content.csv invalid date of birth at row 2"
                                                        ).and have_selector(
                                                                ".govuk-error-message",
                                                                text:
                                                                  "invalid_content.csv invalid period start date at row 2"
        ...
        expect(page).to have_selector(
          ".govuk-error-summary__body",
          text:
            "invalid_period.csv period end date earlier than period start date at row 2"
        ).and have_selector(
                ".govuk-error-message",
                text:
                  "invalid_period.csv period end date earlier than period start date at row 2"
