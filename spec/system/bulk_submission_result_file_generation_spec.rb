...
RSpec.describe "Generation of a result file", type: :worker do
  ...
  let(:row_as_hash) do
    csv[0].to_h.except("uc_one_data", "uc_two_data").symbolize_keys
  end
  let(:csv) do
    CSV.parse(bulk_submission.result_file.download, headers: :first_row)
  end
  ...
  context "with any completed hmrc result" do
    ...
    it "result file contains expected headers" do
      perform_inline
      expect(csv.headers).to match(expected_headers),
    ...
    it "result file contains details from original_file" do
      perform_inline
      expect(row_as_hash).to match(hash_including(expected_row_as_hash)),
