...
RSpec.describe HmrcInterface::Client do
  ...
  before do
    allow(HmrcInterface.configuration).to receive(:host).and_return(
      "https://fake-laa-hmrc-interface.service.justice.gov.uk"
    )

    stub_request(
      :post,
      %r{(http|https).*laa-hmrc-interface.*/oauth/token}
    ).to_return(
      status: 200,
      body:
  ...
  describe "#headers" do
    ...
    it "includes expected headers" do
      expect(headers).to include("Accept" => "application/json").and include(
              "Content-Type" => "application/json"
            ).and include(
                    "Authorization" => "Bearer test-bearer-token"
                  ).and include(
                          "User-Agent" =>
                            /laa-hmrc-interface-client\/\d{1}\.\d{1}\.\d{1}$/
                        )
    end
  end
  ...
  describe "#access_token" do
    ...
    it "sends custom user agent in header" do
      ...
      expect(
        a_request(
          :post,
          "https://fake-laa-hmrc-interface.service.justice.gov.uk/oauth/token"
        )
      ).to have_been_made
    end
    ...
    context "when retrieving a new token" do
      let(:oauth_client) do
        instance_double(::OAuth2::Client, client_credentials:)
      end
      ...
      let(:new_token) do
        instance_double(::OAuth2::AccessToken, token: "new-fake-token")
      end
      ...
      context "when token nil?" do
        before { client.instance_variable_set(:@access_token, nil) }
        ...
        it "retrieves new access_token using client_credentials grant type" do
          ...
          expect(client_credentials).to have_received(:get_token).with(
      ...
      context "when token expired?" do
        ...
        before { client.instance_variable_set(:@access_token, old_token) }
  ...
  describe "#bearer_token" do
    ...
    context "when test_mode not set" do
      before do
        allow(HmrcInterface.configuration).to receive(:test_mode?).and_return(
          nil
        )
      end
