...
RSpec.describe User, type: :model do
  describe "#email" do
    ...
    it "is case insensitive when queried" do
      query_email = email
      expect(
        described_class.find_by(email: query_email).email
  ...
  describe ".from_omniauth" do
    ...
    context "when user exists with auth_subject_uid" do
      ...
      let(:user) do
        described_class.create!(auth_subject_uid: "jim-bob-fake-uid")
      end
    ...
    context "when user exists without auth_subject_uid" do
      ...
      it "updates auth_subject_id, first_name, last_name" do
        expect { call }.to change { user.reload.attributes }.from(
          hash_including(
            "auth_subject_uid" => nil,
            "first_name" => "",
            "last_name" => ""
  ...
  describe "#full_name" do
    ...
    let(:instance) do
      described_class.new(first_name: " Jim", last_name: "BOB  ")
    end
