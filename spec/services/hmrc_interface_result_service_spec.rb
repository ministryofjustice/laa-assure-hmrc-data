...
RSpec.describe HmrcInterfaceResultService do
  ...
  describe "#call" do
    ...
    context "when result has completed status" do
      ...
      it "updates submission hmrc_interface_result to parsed response body", :aggregate_failures do
        expect { call }
          .to change { JSON.parse(submission.reload.hmrc_interface_result.to_json, symbolize_names: true) }
                .from("{}")
                .to(parsed_response)
      end
    end
    ...
    context "when result has processing status" do
      ...
      let(:parsed_processing_response) do
        { status: "processing" }
      end
      ...
      it "updates status to processing and hmrc_interface_result to returned payload" do
        ...
        expect(submission.reload.hmrc_interface_result).to eql(parsed_processing_response.as_json)
      end
    end
