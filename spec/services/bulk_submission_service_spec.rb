  ...
  describe "#call" do
    ...
    context "when bulk submission has valid original_file attached" do
      ...
      it "creates submissions with expected attributes" do
        ...
        submissions =
          Submission.all.map { |e| e.attributes.with_indifferent_access }
      ...
      it "updates bulk_submission status to \"prepared\"" do
        expect { call }.to change { bulk_submission.reload.status }.from(
          "pending"
        ).to("prepared")
      end

      it "enqueues HmrcInterfaceBulkSubmissionWorker job for bulk_submission",
         type: :worker do
        expect { call }.to change(
          HmrcInterfaceBulkSubmissionWorker,
          :jobs
        ).from([]).to(
          [
            hash_including(
      ...
      it "enqueues BulkSubmissionStatusWorker job for bulk_submission with delay",
         type: :worker do
        allow(BulkSubmissionStatusWorker).to receive(
          :perform_in
        ).and_call_original

        expect { call }.to change(BulkSubmissionStatusWorker, :jobs).from(
          []
        ).to(
          [
            hash_including(
        ...
        expect(BulkSubmissionStatusWorker).to have_received(:perform_in).with(
          28.seconds,
          bulk_submission.id
        )
      end
